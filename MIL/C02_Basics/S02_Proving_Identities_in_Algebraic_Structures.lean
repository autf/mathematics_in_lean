...
theorem mul_inv_cancel (a : G) : a * a⁻¹ = 1 := by
  -- rewrite the first subexpression matching pattern `a`
  conv in a => rw [← inv_inv a]
...
theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  have hbi : b⁻¹ = (a * b)⁻¹ * a
  · apply (mul_left_inj b).mp
    rw [mul_assoc, inv_mul_cancel, inv_mul_cancel]
  have hai : a⁻¹ = b * (a * b)⁻¹
  · apply (mul_right_inj a).mp
    rw [← mul_assoc, mul_inv_cancel, mul_inv_cancel]
  rw [hbi, hai, mul_assoc, ← mul_assoc a b, mul_inv_cancel, mul_one]
